...
import"./main-R5KTlRTc.js";import{s as i,a as I}from"./modal-CQ86hjxP.js";const L="modulepreload",C=function(t){return"/"+t},b={},w=function(o,e,a){let s=Promise.resolve();if(e&&e.length>0){let n=function(d){return Promise.all(d.map(p=>Promise.resolve(p).then(v=>({status:"fulfilled",value:v}),v=>({status:"rejected",reason:v}))))};document.getElementsByTagName("link");const r=document.querySelector("meta[property=csp-nonce]"),h=(r==null?void 0:r.nonce)||(r==null?void 0:r.getAttribute("nonce"));s=n(e.map(d=>{if(d=C(d),d in b)return;b[d]=!0;const p=d.endsWith(".css"),v=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${v}`))return;const l=document.createElement("link");if(l.rel=p?"stylesheet":L,p||(l.as="script"),l.crossOrigin="",l.href=d,h&&l.setAttribute("nonce",h),document.head.appendChild(l),p)return new Promise((T,A)=>{l.addEventListener("load",T),l.addEventListener("error",()=>A(new Error(`Unable to preload CSS for ${d}`)))})}))}function c(n){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=n,window.dispatchEvent(r),!r.defaultPrevented)throw n}return s.then(n=>{for(const r of n||[])r.status==="rejected"&&c(r.reason);return o().catch(c)})};let f=[];const g=localStorage.getItem("nome"),m=localStorage.getItem("turmaSelecionada");g&&(document.getElementById("nomeTreinador").textContent=g);m&&(document.getElementById("turmaSelecionada").textContent=m);async function y(){try{f=await(await fetch(`http://localhost:3000/api/aluno/buscar?turma=${encodeURIComponent(m)}`)).json();const o=document.getElementById("lista-alunos");o.innerHTML='<h4 style="margin-top: 0; color: #142191;">Nome dos Alunos</h4>',f.forEach((e,a)=>{const s=document.createElement("div");s.className="aluno-card",s.innerHTML=`
        <span>${u(e.nome)}</span>
        <span>
  ...
  `;const{Modal:a}=await w(async()=>{const{Modal:c}=await import("./main-R5KTlRTc.js").then(n=>n.b);return{Modal:c}},__vite__mapDeps([0,1])),s=new a(document.getElementById("editModal"));s.show(),document.getElementById("confirmarEdicao").addEventListener("click",async()=>{const c=document.getElementById("inputNovoNome").value.trim();if(!c)return i("Erro","O nome não pode ser vazio.");try{const n=await fetch(`http://localhost:3000/api/aluno/editar/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({nome:c})}),r=await n.json();s.hide(),n.ok?(i("Sucesso",r.message),y()):i("Erro",r.error)}catch{s.hide(),i("Erro","Erro ao editar aluno.")}})}async function P(t,o){if(await I("Remover Aluno",`Tem certeza que deseja remover o aluno <strong>${u(o)}</strong>?`))try{const a=await fetch(`http://localhost:3000/api/aluno/remover/${t}`,{method:"DELETE"}),s=await a.json();a.ok?(i("Sucesso",s.message),y()):i("Erro",s.error)}catch{i("Erro","Erro ao remover aluno.")}}async function j(){const t=document.getElementById("dataSelecionada").value;if(!t)return i("Atenção","Selecione uma data!");const o={};f.forEach((e,a)=>{const s=document.querySelector(`input[name=aluno${a}]:checked`);o[e.nome]=s?s.value:null});try{const a=await(await fetch("http://localhost:3000/api/presenca/registrar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({turma:m,data:t,registros:o})})).json();i("Informação",a.message||"Presenças salvas!"),E()}catch{i("Erro","Erro ao salvar presença.")}}async function B(t,o,e){const a=document.getElementById("modal-container");a.innerHTML=`
  ...
  `;const{Modal:s}=await w(async()=>{const{Modal:n}=await import("./main-R5KTlRTc.js").then(r=>r.b);return{Modal:n}},__vite__mapDeps([0,1])),c=new s(document.getElementById("editPresencaModal"));c.show(),document.getElementById("confirmarEdicaoPresenca").addEventListener("click",async()=>{const n=document.getElementById("novaPresenca").value;try{const r=await fetch("http://localhost:3000/api/presenca/editar",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({turma:m,data:o,nomeAluno:t,novaPresenca:n})}),h=await r.json();c.hide(),r.ok?(i("Sucesso",h.message||"Presença atualizada."),E()):i("Erro",h.error||"Erro ao atualizar presença.")}catch{c.hide(),i("Erro","Erro ao atualizar presença.")}})}async function E(){const t=document.getElementById("historico");t.innerHTML="";try{const e=await(await fetch(`http://localhost:3000/api/presenca/historico?turma=${encodeURIComponent(m)}`)).json();if(!e||Object.keys(e).length===0){t.innerHTML="<p>Nenhum histórico encontrado.</p>";return}f.forEach(a=>{const s=Object.entries(e).filter(([c,n])=>n[a.nome]).map(([c,n])=>({data:c,status:n[a.nome]}));s.length&&(t.innerHTML+=`
          <details>
