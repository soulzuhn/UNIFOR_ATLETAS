import"./main-R5KTlRTc.js";document.addEventListener("DOMContentLoaded",async()=>{const a=localStorage.getItem("turmaSelecionada");document.getElementById("nomeTurma").textContent=a;try{const i=await(await fetch(`http://localhost:3000/api/presenca/historico?turma=${encodeURIComponent(a)}`)).json(),s=document.querySelector("#tabela-frequencia tbody");s.innerHTML="";let e=0,t=0,n=0;Object.entries(i).forEach(([u,m])=>{Object.entries(m).forEach(([g,o])=>{e++,o.toLowerCase()==="presente"?t++:o.toLowerCase()==="ausente"&&n++;const c=document.createElement("tr");c.innerHTML=`
